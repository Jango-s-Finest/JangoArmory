...
_damageScript = "FSN\freestyleNuke\crater.sqf";
if(_ace) then {_damageScript = "FSN\freestyleNuke\aceDamage.sqf"};
...
while {((((getPos (_allBuildings # _c)) distance _blastPos) - 40) <= _curRadius5) && (_c < _maxB)} do
{
	...
	if (_h > ((_curRadius5 - 40) max _rad20psi) && _h <= _curRadius5) then {
		...
		if ((random 100) < 5 && (_x isKindOf "House")) then{
			[_x,2,300 + (random 60), _ace] execVM "FSN\freestyleSimpleFire\iniFire.sqf"; //set buildings on fire (approx. 4% of all buildings affected)
		};
	};
	_c = _c + 1;
};
