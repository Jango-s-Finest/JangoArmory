...
[[_blastPos, _radFireball, _airMode],"FSN\freestyleNuke\flash.sqf"] remoteExec ["execVM",0];


if(! _airMode) then 
{
	[[_blastPos, _radFireball,_rad5psi * 0.8,_rad5psi * 1.1],"FSN\freestyleNuke\mushroomcloud.sqf"] remoteExec ["execVM",0];
};

[[_blastPos, _rad1psi, _airMode],"FSN\freestyleNuke\blastwave.sqf"] remoteExec ["execVM",0];

//[[_blastPos, 0.7 * _radFireball,_rad5psi * 0.8,_rad5psi * 1.1],"FSN\freestyleNuke\mushroomcloud.sqf"] remoteExec ["execVM",0];
//[[_blastPos, 0.7 * _radFireball,_radFireball * 6,_radFireball * 8.5],"FSN\freestyleNuke\mushroomcloud.sqf"] remoteExec ["execVM",0]; //legacy function


if (_radFireball > 46) then
{
	[_blastPos, _y, _radFireball * 2, _airMode] execVM "FSN\freestyleNuke\iniCondensationRings.sqf";
}
else
{
	[_blastPos, -1, _radFireball * 2, _airMode] execVM "FSN\freestyleNuke\iniCondensationRings.sqf";
};


if((! _airMode) && ((_radFireball / 0.4)  > (_blastPos # 2))) then
{
	[[_blastPos, _radFallout, 900],"FSN\freestyleNuke\falloutParticle.sqf"] remoteExec ["execVM",0];
};

//create damaging effects
if (_damage) then {
	_5000rem = [_blastPos, _rad5000rem, _aceActivated] execVM "FSN\freestyleNuke\radioation5000rem.sqf";
	_500rem = [_blastPos, _rad500rem, _aceActivated] execVM "FSN\freestyleNuke\radioation5000rem.sqf";
	_100thermal = [_blastPos, _rad100thermal, _aceActivated] execVM "FSN\freestyleNuke\thermal100.sqf";
	_50thermal = [_blastPos, _rad50thermal, _aceActivated] execVM "FSN\freestyleNuke\thermal50.sqf";

	
	/*sleep 5;
	private _start = diag_tickTime;*/
	
	_20psi = [_blastPos, _rad20psi, _radCrater + 10, _aceActivated] execVM "FSN\freestyleNuke\airblast20psi.sqf";
	waitUntil { scriptDone _20psi };
	
	
	if(! _airMode) then 
	{
		_crater = [_blastPos, _radCrater, _aceActivated] execVM "FSN\freestyleNuke\crater.sqf";	
	};
	
	_5psi = [_blastPos, _rad5psi,_rad20psi, _aceActivated] execVM "FSN\freestyleNuke\airblast5psi.sqf";
	waitUntil { scriptDone _5psi };

	_1psi = [_blastPos, _rad1psi,_rad5psi, _aceActivated] execVM "FSN\freestyleNuke\airblast1psi.sqf";
	waitUntil { scriptDone _1psi };

	//hint str(diag_tickTime - _start);
	
	_jam = [_blastPos, _rad1psi] execVM "FSN\freestyleNuke\jamming.sqf";
	
	if ((! _airMode) && ((_radFireball / 0.4)  > (_blastPos # 2)) && (FSN_Fallout)) then
	{
		_waste = [_blastPos, _radFallout, 900, _uniforms, _goggles, _aceActivated] execVM "FSN\freestyleNuke\nuclearWaste.sqf";
	};
}
