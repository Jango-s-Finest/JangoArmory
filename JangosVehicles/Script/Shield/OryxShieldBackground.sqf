...
_vic addAction [
        "Enable Shield",
        {
			...
			_target setVariable ["104thOryxShieldStrength", 1000];
            ...
            _index = _target addEventHandler ["HitPart", { 
				...
				systemChat ("Ammo array: " + str(_ammo));
				
				_damage = 0;
				_damagedone = 0;
				if (_isDirect) then {_damage = (_ammo select 0)} else {_damage = ((_ammo select 1) / (_ammo select 2))};
				
				systemChat ("damage " + str(_damage));
				
				
				if (_damage > 29) then {_damagedone = 10};
				if (_damage > 100) then {_damagedone = 50};
				if (_damage > 300) then {_damagedone = 300};
				if (_damage > 999) then {_damagedone = 1000};
				systemChat ("damagedone " + str(_damagedone));
				
				
				_shieldadjust = _targetvehicle getVariable "104thOryxShieldStrength";
				_shieldadjust = (_shieldadjust - _damagedone);
				_targetvehicle setVariable ["104thOryxShieldStrength", _shieldadjust];
				systemChat ("Shield Power " + str(_shieldadjust));
				...
				systemChat ("About to spawn " + str(_vehicletospawn));
            ...
            waitUntil { sleep 1; _shieldcheck = _target getVariable "104thOryxShieldStrength"; _shieldcheck <= 0 };
            ...
            sleep 30;
            [_target] execVM "\JangosVehicles\Script\Shield\OryxShieldBackground.sqf";
        },
