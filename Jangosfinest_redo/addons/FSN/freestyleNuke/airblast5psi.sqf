...
private ["_h", "_curDamage5","_mark20psi", "_abort", "_house", "_yVel", "_xVel", "_mass"];


private _blastPos = _this select 0;
private _radius5 = _this select 1;
private _rad20psi = _this select 2;
private _curRadius5 = 40 + _rad20psi;
private _ace = param[3, false];
...
private _allTrees = nearestTerrainObjects [_blastPos, ["SMALL TREE", "BUSH"], _radius5];
private _allBuildings = nearestObjects [_blastPos ,["Building"], _radius5];
private _allVehicles = nearestObjects [_blastPos ,["Car", "LandVehicles", "Air", "Ship"], _radius5];
private _allUnits = nearestObjects [_blastPos ,["Man"], _radius5];
...
while {((((getPos (_allBuildings # _c)) distance _blastPos) - 40) <= _curRadius5) && (_c < _maxB)} do
{
	...
	if (_h > ((_curRadius5 - 40) max _rad20psi) && _h <= _curRadius5) then {
		...
		if ((random 100) < 5 && (_x isKindOf "House")) then{
			[_x,2,300 + (random 60), _ace] execVM "JA\jangos_Armory\addons\FSN\freestyleSimpleFire\iniFire.sqf"; //set buildings on fire (approx. 4% of all buildings affected)
		};
	};
	_c = _c + 1;
};
