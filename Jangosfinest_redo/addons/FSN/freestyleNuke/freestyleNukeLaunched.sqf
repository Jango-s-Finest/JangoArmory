private ["_blastPos", "_mode", "_dud"];


private _ammo = param [0, "any"];
private _magazine = param [1, "any"];
private _projectile = param [2];
...
private _yield = switch (_mode) do
		{
			case "A": {0.3};
			case "B": {1.5};
			case "C": {10};
...
private _valid = switch (_magazine) do
		{
...
then 
{
	...
	[_blastPos, _yield, false, true, 2] execVM "JA\jangos_Armory\addons\FSN\freestyleNuke\iniNuke.sqf";
