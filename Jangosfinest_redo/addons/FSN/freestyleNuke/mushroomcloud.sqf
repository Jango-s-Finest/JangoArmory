...
private["_cloudRadius", "_lifetime", "_cap", "_capLifetime", "_h"];

if (!hasInterface) exitWith {};

private _pos = _this select 0;
private _radius = 1.5 * (_this select 1);
private _cloudBottom = _this select 2;
private _cloudTop = _this select 3;
private _cloudLifetime = 300;

private _color = [1,1,1,1];
private _colorTarget = [1,1,1,0.7];

private _vSpeed = 20;

private _obj = "Sign_Sphere10cm_F" createVehicleLocal _pos;
private _capObj = "Sign_Sphere10cm_F" createVehicleLocal [_pos select 0, _pos select 1,  (_pos select 2) + _radius * 2];
