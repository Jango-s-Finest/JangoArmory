...
private _yield = _this select 1;
private _radius = _this select 2;
_radius = _radius * 5;
private _airMode = _this select 3;
...
{
	[[_blastPos, _radius * 0.5 + _i * _radius * 0.1, _radius - (_i + 1) * _radius * 0.1],"JA\jangos_Armory\addons\FSN\freestyleNuke\condensationRing.sqf"] remoteExec ["execVM",0];
	sleep 0.5;
};
	}
	else
...
default {0};
	...
	[[_blastPos, _i * _radius * 0.1, _radius - (_i + 1) * _radius * 0.1],"JA\jangos_Armory\addons\FSN\freestyleNuke\condensationRing.sqf"] remoteExec ["execVM",0];
	sleep 0.5;
};
	};
};
