//Changed in 0.7.0
//Changed in 0.6.0
//Created in 0.2.0


private ["_allUnits", "_t"];

private _blastPos = _this select 0;
private _radius = _this select 1;
private _duration = _this select 2;
private _protectUniforms = _this select 3;
private _protectGoggles = _this select 4;
private _ace = param[5, false];



_damageScript = "JA\jangos_Armory\addons\FSN\freestyleNuke\crater.sqf";
if(_ace) then {_damageScript = "JA\jangos_Armory\addons\FSN\freestyleNuke\aceDamage.sqf"};


private _fullDuration = _duration;


while {_duration > 0} do
{
	_allUnits = _blastPos nearObjects ["Man", _radius]; //get affected units
	
	//damage units depending on thier protective gear
	{ 
		if(!(((uniform _x) in _protectUniforms) || ("CRBN" in (uniform _x)) || ("crbn" in (uniform _x)) || ("hazmat" in (uniform _x))) && isDamageAllowed _x) then 
		{
			if (_ace) then 
				{
					[_x, 0.2, "ropeburn"] execVM _damageScript;
				}
				else 
				{
					_x setDamage [damage _x + 0.04, true];
				};			
		};
		if(!((goggles _x) in _protectGoggles || ("CRBN" in (goggles _x)) || ("crbn" in (goggles _x)) || ("respirator" in (goggles _x)) || ("Respirator" in (goggles _x)) || ("hazmat" in (goggles _x))) && isDamageAllowed _x) then 
		{
			if (_ace) then 
				{
					[_x, 0.1, "ropeburn"] execVM _damageScript;
				}
				else 
				{
					_x setDamage [damage _x + 0.01, true];
				};	
		};
		
		if(isPlayer _x) then
		{
			[[_duration, _fullDuration],"JA\jangos_Armory\addons\FSN\freestyleNuke\detector.sqf"] remoteExec ["execVM",_x];
		};
		
	} forEach _allUnits;
	
	

	
	_t = random[1,5,10]; //wait random time to next tick
	sleep _t;
	_duration = _duration - _t;
};


