		...
		for [{_i = 0}, {_i < (count _vehicles)}, {_i = _i + 1}] do {
			...
			while {(_health >= 0.00) &&(_target distance _droid < _range)} do {
				if ((_target distance _droid < _range) && (!lineIntersects [getPos _droid,getPos _target])) then{
					...
					[_target, (_health)] remoteExecCall ["ace_repair_fnc_setDamage"];
					sleep 0.5;
				};
			};
			[_target, 0] remoteExecCall ["setDamage"];
			_fuel = fuel _target;
			while {(_fuel <= 1)&&(_target distance _droid < _range)} do {
				if ((_target distance _droid < _range) && (!lineIntersects [getPos _droid,getPos _target])) then{
					_fuel = fuel _target;
					_fuel = _fuel + 0.01;
					[_target, _fuel] remoteExecCall  ["setFuel"];
					sleep 0.5;
				};
			};
			[_target, 1] remoteExecCall  ["setFuel"];
